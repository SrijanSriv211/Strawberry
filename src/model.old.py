...
class Config:
	...
	block_size: int = 1024
	f_retention: int = 8
	r_layer: int = 2 # retention layers
...
class RetentionMechanism(nn.Module):
	def __init__(self, config: Config):
		...
		self.f_retention = config.f_retention
		self.n_embd = config.n_embd
		self.n_qkv = config.n_qkv

		self.lr_embd = config.n_embd // config.f_retention # low-rank embedding size
		self.swiglu = CastedLinear(config.n_embd, self.lr_embd*2)
		self.out = CastedLinear(self.lr_embd, config.n_embd)

		self.w_attn_qkv = CastedLinear(config.n_embd, config.n_qkv*3)
		self.w_attn_out = CastedLinear(config.n_qkv, config.n_embd)
		...
		self.m = torch.arange(1, (4 * self.f_qkv)/2).unsqueeze(1).unsqueeze(1)
	...
	def forward(self, x):
		...
		# (16, R, R) -> (4*D, C)
		y = self.out(y)
		if self.f_retention > 1:
			y = y.repeat_interleave(self.f_retention, dim=1)
		...
		# normalize
		w_qkv = self.w_attn_qkv.weight + F.normalize(w_qkv, dim=0) * self.n_embd**-0.5
		w_out = self.w_attn_out.weight + F.normalize(w_out.T, dim=0) * self.n_qkv**-0.5
		return w_qkv, w_out
...
class TheExpertAbundance(nn.Module):
	...
	def forward(self, x, cos_sin, i):
		# batch size, sequence length, embedding dimensionality (n_embd)
		B, T, _ = x.size()

		# dynamically generate w_qkv & w_out weights based on input `x`
